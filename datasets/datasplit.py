...
class DataSplit:
    ...
    def load_node_split(self, split_i):
        ...
        return {key: torch.load(path, weights_only=True).long() for key, path in split_paths.items()}
    ...
    def load_edge_split(self, split_i):
        ...
        # Edge split files store full PyG Data objects, not just tensors.
        # PyTorch >=2.6 defaults torch.load(..., weights_only=True), which
        # blocks unpickling these objects unless explicitly disabled.
        return {key: torch.load(path, weights_only=False) for key, path in split_paths.items()}
